...
// Mode Demo - set ke false untuk menggunakan Supabase
const DEMO_MODE = config.isDemoMode;
...
function setupEventListeners() {
    ...
    // Perbaiki event listener untuk tombol yang benar
    const addButton = document.querySelector('.btn-danger'); // Tombol "Hapus" sebenarnya untuk add
    if (addButton) {
        addButton.textContent = '➕ Tambah'; // Ubah text menjadi "Tambah"
        addButton.className = 'btn btn-success'; // Ubah class menjadi success
...
async function loadSiswaData() {
    ...
    try {
        // Jika dalam mode demo atau Supabase tidak dikonfigurasi, gunakan dummy data
        if (DEMO_MODE || !supabase) {
            console.log('Using dummy data');
            allData = getDummyData();
            filteredData = [...allData];
            totalItems = allData.length;
            console.log('Dummy data loaded:', allData);
            renderTable();
            updatePagination();
            return;
        }
...
async function saveSiswa(siswaData) {
    try {
        // Jika dalam mode demo atau Supabase tidak dikonfigurasi
        if (DEMO_MODE || !supabase) {
            if (editingId) {
...
// Export functions for Excel and PDF (simplified versions)
